...
class index extends foreground {
	...
	public function register() {
		...
		if(isset($_POST['dosubmit'])) {
			...
			if (empty($_POST['username'])||empty($_POST['email'])||empty($_POST['password'])||empty($_POST['nickname'])) {
				showmessage(L('infomation_is_incomplete'),'index.php?m=member&c=index&a=login');
			}
			$userinfo = array();
	...
	public function login() {
		...
		if(isset($_POST['dosubmit'])) {
			...
			unset($_SESSION['connectid'], $_SESSION['from']);
			//将从腾讯或新浪微博接口返回的openid和来源（qq,sina）更新到phpcms的用户表中
	...
	public function public_sina_login() {
		...
		define('WB_CALLBACK', pc_base::load_config('system', 'sina_callback'));
		...
		if(isset($_REQUEST['code'])) {
			...
			try {
				...
				showmessage(L('authorize_failure'), 'index.php?m=member&c=index&a=login');
			}
			if ($token) {
			...
			if(!empty($uid)) {
				...
				$where = array('connectid'=>$uid, 'from'=>'sina');
				...
				if(!empty($r)) {
					...
					$groupid = $r['groupid'];
					$username = $r['username'];
					if(!empty($me['screen_name'])){
						$nickname=$me['screen_name'];
					}else {
						$nickname = empty($r['nickname']) ? $username : $r['nickname'];
					}
					
					$updatearr = array('lastip'=>ip(), 'lastdate'=>SYS_TIME,'nickname'=>$nickname);
					//vip过期，更新vip和会员组
					if($r['overduedate'] < SYS_TIME) {
						$updatearr['vip'] = 0;
					}
			...
			$o = new SaeTOAuthV2(WB_AKEY, WB_SKEY);
			$aurl = $o->getAuthorizeURL( WB_CALLBACK );
    ...
    public function public_qq_loginnew(){
    	...
    	if(!isset($_GET['code'])){
    		...
    		if(!empty($_SESSION['openid'])){
    			$user = $info->get_user_info($_SESSION['access_token'], $appid,$_SESSION['openid'] );
    			//根据从腾讯接口返回的openid,在phpcms用户表中查询该QQ用户是否完善本网站上的信息或是否与本网站的注册帐号绑定
