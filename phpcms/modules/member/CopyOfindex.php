...
class index extends foreground {
	...
	public function public_sina_login() {
		$appid = pc_base::load_config('system', 'sina_akey');
		$appkey = pc_base::load_config('system', 'sina_skey');
		$callback = "http://www.inave.cn/index.php?m=member&c=index&a=public_sina_login";
		pc_base::load_app_class('weibooauth', '' ,0);
		$info = new sinaapi($appid, $appkey,$callback);
		$cookietime = intval($_POST['cookietime']);
		$this->_session_start();
		if(!isset($_GET['code'])){
					...
					if(!empty($_SESSION['openid'])){
						...
						$r = $this->db->get_one(array('connectid'=>$_SESSION['openid'],'from'=>'qq'));
						 if(!empty($r)){
								...
								$username = $r['username'];
								$nickname = empty($r['nickname']) ? $username : $r['nickname'];
								$this->db->update(array('lastip'=>ip(), 'lastdate'=>SYS_TIME, 'nickname'=>$user->nickname), array('userid'=>$userid));
								if(!$cookietime) $get_cookietime = param::get_cookie('cookietime');
								$_cookietime = $cookietime ? intval($cookietime) : ($get_cookietime ? $get_cookietime : 0);
								$cookietime = $_cookietime ? TIME + $_cookietime : 0;
								$phpcms_auth_key = md5(pc_base::load_config('system', 'auth_key').$this->http_user_agent);
